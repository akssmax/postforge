...
function buildSnapshotPrompt(snapshot: DesignSnapshot): string {
  ...
  return [
    ...
    `- Text slots: ${snapshot.textSlots.map((s) => `${s.slotId}[${s.role}]=${JSON.stringify(s.text.slice(0, 60))}`).join("; ")}`,
    `- updateCopy slotId must be one of: ${snapshot.textSlots.map((s) => s.slotId).join(", ")} (or role aliases headline/subheading)`,
